...
void NX_VPU_HwOn(void)
{
	...
	pVpuBaseAddr = ioremap(0xC0080000, 0x4000);
	...
	WriteReg32( pNPreCharge, tmpVal );		//	Pre Charge
...
void NX_VPU_HWOff(void)
{
	...
	if( gstIsVPUOn )
	{
		...
		tmpVal = ReadReg32( pIsolateBase );
		tmpVal &= (~POWER_PMU_VPU_MASK);
		WriteReg32( pIsolateBase,   tmpVal );		//	Isolate
		...
		//	Isolate VPU H/W
		// tmpVal = VpuReadReg( pIsolateBase );
		// tmpVal &= (~POWER_PMU_VPU_MASK);
		// VpuWriteReg( pIsolateBase,   tmpVal );		//	Isolate
...
static uint32_t *gstCodaClockEnRegVir = NULL;
...
int VPU_InitInterrupt(void)
{
	...
	//	Initialize Interrupt
	if(request_irq( VPU_IRQ_ID, VPU_InterruptHandler, 0, "VPU_CODEC_IRQ", NULL) != 0)
	{
		printk("%s:%d Cannot get IRQ %d\n", __func__, __LINE__, VPU_IRQ_ID );
		return -1;
	}
...
//	VPU Initialization.
//----------------------------------------------------------------------------
NX_VPU_RET	NX_VpuInit( void *firmVirAddr, uint32_t firmPhyAddr )
{
	...
	for( i=0 ; i<NX_MAX_VPU_INST_SPACE ; i++ )
	{
		...
		gstVpuInstance[i].paramVirAddr = (uint64_t)(firmVirAddr + CODE_BUF_SIZE + TEMP_BUF_SIZE);
...
NX_VPU_RET	NX_VpuEncSetSeqParam( NX_VPU_INST_HANDLE handle, VPU_ENC_SEQ_ARG *seqArg )
{
	...
	NX_DbgMsg(INFO_MSG, ("NX_VpuEncSetSeqParam() : %dx%d, %d/%d fps, %d kbps (gop(%d), maxQ(%d), SR(%d), StreamBuffer(0x%llx, 0x%llx))\n",
...
static NX_VPU_RET VPU_EncSeqCommand(NX_VpuCodecInst *pInst)
{
	...
	// printk("pEncInfo->strmBufPhyAddr = 0x%llx\n", pEncInfo->strmBufPhyAddr);
	// printk("pEncInfo->strmBufSize = 0x%08x\n", pEncInfo->strmBufSize/1024);
...
static int FillBuffer(NX_VPU_INST_HANDLE handle, unsigned char *stream, int size)
{
	...
	{
		printk("%s, StreamBuffer(Addr=x0%08x, size=%d), InBuffer(Addr=%p, size=%d) vWriteOffset = %d, vReadOffset = %d\n",
			__func__, (uint32_t)pDecInfo->strmBufVirAddr, pDecInfo->strmBufSize, stream, size, vWriteOffset, vReadOffset );
		return -1;
	}
	...
	{
		//	Just Memory Copy
		if( copy_from_user( (uint8_t*)(pDecInfo->strmBufVirAddr + vWriteOffset), stream, size ) )
			return -1;
		vWriteOffset += size;
	}
	else
	{
		...
		if( copy_from_user( (uint8_t*)(pDecInfo->strmBufVirAddr + vWriteOffset), stream, remain ) )
			return -1;
		if( copy_from_user( (uint8_t*)(pDecInfo->strmBufVirAddr), stream+remain, size-remain    ) )
			return -1;
...
//	VpuDecInfo *pInfo = &handle->codecInfo.decInfo;
	...
	//	FillBuffer
	if( 0 > FillBuffer( handle, (uint8_t*)seqArg->seqData, seqArg->seqDataSize ) )
	{
...
NX_VPU_RET	NX_VpuDecRunFrame( NX_VPU_INST_HANDLE handle, VPU_DEC_DEC_FRAME_ARG *decArg)
{
	...
	//	Fill Data
	if( 0 > FillBuffer( handle, (uint8_t *)decArg->strmData, decArg->strmDataSize ) )
	{
...
//		printk("Need More Buffer!!!!!\n");
	...
	if(pInfo->userDataEnable) {
		...
		NX_DbgMsg( DBG_USERDATA, (" User Data : Buffer(0x%08x), Size(%d)\n", (uint32_t)pInfo->userDataBufPhyAddr, pInfo->userDataBufSize) );
	}
	else {
