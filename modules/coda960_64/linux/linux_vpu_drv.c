...
//	Instance Management
static int32_t	gstCurNumInstance = 0;		//	Maximum Instances         : NX_MAX_VPU_INSTANCE
static int32_t	gstCruEncInstance = 0;		//	Maximum Encoder Instances : NX_MAX_VPU_ENC_INST	//	0~1
static int32_t	gstCruDecInstance = 0;		//	Maximum Decoder Instances : NX_MAX_VPU_DEC_INST	//	2~3
static int32_t	gstCurJPGInstance = 0;		//	Maximum JPG Enc Instances : NX_MAX_VPU_JPG_INST	//	4

//	Firmware Physical & Virtual Address
static dma_addr_t	gstFirmPhyAddress = 0;		//	Fimmware Physical Address
static uint32_t		*gstFirmVirAddress = 0;		//	Firmware Virtual Address
	...
	case IOCTL_VPU_DEC_SET_SEQ_INFO:
		{
			...
			{
				unsigned char *pbuf;
				pbuf = (unsigned char*)seqArg.seqData;
...
static int __init nx_vpu_init(void)
{
	int res = -1;
	...
	NX_LinearAlloc( &nx_vpu_device->dev, (uint32_t*)&gstFirmPhyAddress, &gstFirmVirAddress, COMMON_BUF_SIZE, 4096 );
	NX_DrvMemset( gstFirmVirAddress, 0, COMMON_BUF_SIZE );
...
static void __exit nx_vpu_exit(void)
{
	...
	{
		NX_LinearFree( &nx_vpu_device->dev, gstFirmPhyAddress, (uint32_t*)gstFirmVirAddress );
	}
	DrvMutexUnlock( &gstDrvMutex );

	DrvCloseMutex( &gstDrvMutex );

	NX_DbgMsg( NX_DBG_INFO, ( "Nexell VPU Driver Unloaded\n" ) );
}
