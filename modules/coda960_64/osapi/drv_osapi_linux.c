...
//
void	NX_LinearAlloc( void *handle, uint32_t *phyAddr, uint32_t **virAddr, int32_t size, int32_t align )
{
	...
	*virAddr = (uint32_t*)cma_get_virt( *phyAddr, size, 1 );		//	Get non cachable memory
...
void	NX_LinearFree( void *handle, uint32_t phyAddr, uint32_t *virAddr )
{
	cma_free((dma_addr_t)phyAddr);
}
//
//
//
