...
typedef struct tagVpuEncInfo
{
	...
	uint64_t subSampleAPhyAddr;	//	Subsample buffer A Physicall Address
	uint64_t subSampleBPhyAddr;	//	Subsample buffer A Physicall Address

	//	Mpeg4 Encoder Only
	uint64_t usbSampleDPPhyAddr;	//	Data Partition temporal buffer address
	uint32_t usbSampleDPSize;		//	Data Partition temporal buffer size

	int linear2TiledEnable;

	//	Output Stream Buffer's Address & Size
	uint64_t strmBufVirAddr;		//	stream buffer start address(virtual)
	uint64_t strmBufPhyAddr;		//	stream buffer start address(physical)
...
typedef struct tagNX_VpuCodecInst{
	...
	uint64_t instBufPhyAddr;	//	Physical
	uint64_t instBufVirAddr;	//	Virtual
