...
NX_MEMORY_HANDLE NX_AllocateMemory( int size, int align )
{
	...
	handle->privateDesc = ionAllocFdVar;
	handle->align = align;
	handle->size = size;
...
NX_VID_MEMORY_HANDLE NX_VideoAllocateMemory( int align, int width, int height, int memMap, int fourCC )
{
	...
	handle->privateDesc[0] 	= (uint64_t)luHandle;
...
NX_VID_MEMORY_HANDLE NX_VideoAllocateMemory2( int align, int width, int height, int memMap, int fourCC )
{
	...
	handle->privateDesc[0] 	= (uint64_t)luHandle;
	handle->privateDesc[1] 	= (uint64_t)cbHandle;
	handle->privateDesc[2] 	= (uint64_t)crHandle;
	handle->align 			= align;
	handle->memoryMap		= memMap;
	handle->fourCC			= fourCC;
	handle->imgWidth		= width;
...
void NX_FreeVideoMemory( NX_VID_MEMORY_HANDLE handle )
{
	if( handle )
	{
		if( 0 != handle->privateDesc[0] )	NX_FreeMemory( (NX_MEMORY_HANDLE)handle->privateDesc[0] );
		if( 0 != handle->privateDesc[1] )	NX_FreeMemory( (NX_MEMORY_HANDLE)handle->privateDesc[1] );
		if( 0 != handle->privateDesc[2] )	NX_FreeMemory( (NX_MEMORY_HANDLE)handle->privateDesc[2] );
		free(handle);
	}
}
